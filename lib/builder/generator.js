...
export default class Generator {
  ...
  async generateRoutes(routes) {
    ...
    if (!this.options.generate.concurrenceMethod || this.options.generate.concurrenceMethod === 'current') {
    ...
    } else if(this.options.generate.concurrenceMethod === 'ifInterval') {
     // Start generate process
      while (routes.length) {
        let n = 0
        await Promise.all(
          routes
            .splice(0, this.options.generate.concurrency)
            .map(async ({ route, payload }) => {
              if (this.options.generate.interval) {
                await waitFor(n++ * this.options.generate.interval)
              }
              await this.generateRoute({ route, payload, errors })
            })
    ...
    } else if(this.options.generate.concurrenceMethod === 'throttle') {
      let throttled
      if (this.options.generate.interval) {
        throttled = throttle(async ({ route, payload }) => {
          await this.generateRoute({ route, payload, errors })
        }, this.options.generate.interval)
      }
    ...
    } else if(this.options.generate.concurrenceMethod === 'wait') {
      // Start generate process
      let running = 0
      while (routes.length) {
        if (running < this.options.generate.concurrency) {
          routes
            .splice(0, Math.max(0, this.options.generate.concurrency - running))
            .forEach(({ route, payload }) => {
              running++
              this.generateRoute({ route, payload, errors }).then(() => {
                running--
              })
            })
        }
    ...
    } else if(this.options.generate.concurrenceMethod === 'waitthrottled') {
      let throttled
      if (this.options.generate.interval) {
        throttled = throttle(async ({ route, payload }) => {
          await this.generateRoute({ route, payload, errors })
        }, 1, this.options.generate.interval)
    ...
    } else if(this.options.generate.concurrenceMethod === 'race') {
      // Start generate process
      let running = 0
      let p
      while (routes.length) {
      ...
      await Promise.all(p)
      // check a route from a previous Promise.race is not still running
      while (running > 0) {
        await waitFor(1)
      }
    }
